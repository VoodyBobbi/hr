...
# 30 дней. Раньше стоял год — я оптимизировал под «кандидат вернётся
# дозаполнить через полгода», но такого почти не бывает, а цена высокая:
# украденная cookie даёт доступ к переписке кандидата на целый год.
#
# Месяц закрывает реальный сценарий (человек ушёл подумать, вернулся через
# пару недель) и в двенадцать раз сокращает окно для чужого.
