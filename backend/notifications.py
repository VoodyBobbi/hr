...
def send_hr_notification(text: str, buttons: list | None = None) -> None:
    """Отправляет сообщение в HR-группу. Никогда не бросает исключение
    наружу — сбой уведомления не должен ронять сам ответ кандидату (это
    дополнительная, не критичная для работы бота функция).

    buttons — список кнопок вида [{"text": ..., "callback_data": ...}],
    которые Telegram покажет под сообщением. Нажатие обрабатывает
...
def format_new_candidate_notification(card: dict) -> str:
    ...
    return (
        ...
        f"\nНажмите кнопку ниже, чтобы посмотреть все данные. Анкета также лежит в "
        f"зашифрованной таблице candidates.csv (id {_esc(card.get('ID кандидата', '—'))})."
    )


# Поля, которые показываются HR по нажатию кнопки «Показать все данные».
# Порядок — как в анкете, чтобы карточка читалась привычно.
_CARD_ORDER = (
    ...
    "Рост", "Вес", "Размер одежды", "Размер обуви",
    "Судимость", "Административный надзор", "Ограничения по здоровью",
    "Готовность к вахте, когда готов",
    "Паспорт серия", "Паспорт номер", "Кем выдан",
    "Дата выдачи", "Код подразделения выдачи паспорта",
    "СНИЛС", "ИНН",
)


def format_full_card(card: dict) -> str:
    """Полная анкета кандидата — показывается ТОЛЬКО по нажатию кнопки в
    HR-группе, а не в самом уведомлении.

    Разделение намеренное. Уведомление висит в ленте группы постоянно и
    видно всем при беглом пролистывании, в том числе на чужом экране через
    плечо. Паспорт, СНИЛС и ИНН показываются отдельным действием, которое
    HR совершает осознанно, когда действительно берёт кандидата в работу."""
    lines = ["<b>Анкета кандидата</b>", f"ID в таблице: {_esc(card.get('ID кандидата', '—'))}", ""]
    for field in _CARD_ORDER:
        value = str(card.get(field, "")).strip()
        if value:
            lines.append(f"<b>{_esc(field)}:</b> {_esc(value)}")
    return "\n".join(lines)
