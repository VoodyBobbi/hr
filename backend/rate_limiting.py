...
спам-сообщений. Модуль ниже смотрит на ПАТТЕРН поведения одного и того же
кандидата (по source:external_id — та же пара, что использует candidates.py
для идентификации) и начисляет баллы за подозрительные признаки:
...
def record_message(source: str, external_id: str, message: str) -> None:
    ...
    Вызывается ДО is_blocked в app.py — то есть даже сообщение, которое
    только что довело счётчик до порога, само ещё не блокируется (блокирует
    следующее); это осознанный компромисс простоты, не критичный для цели
    модуля (остановить продолжающийся спам, а не поймать самое первое
    сообщение атаки)."""
    key = _key(source, external_id)
    now = time.time()
...
def reset(source: str, external_id: str) -> None:
    """Ручной сброс баллов/блокировки конкретного кандидата — например, для
    случая, когда HR лично убедился, что это не спам, а не сработавшая
    защита на живого человека с необычным паттерном сообщений."""
    key = _key(source, external_id)
    with _lock:
        _state.pop(key, None)
